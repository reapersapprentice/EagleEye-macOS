...
debian_install() {
    sudo brew update && sudo brew upgrade -y
    sudo brew install git python3 python3-pip python3-dev
    sudo brew install libgtk-3-dev libboost-all-dev build-essential cmake libffi-dev
}
...
if [ "$(grep -Ei 'fedora|redhat' /etc/*release)" ]; then
    echo "brew is currently not supported."
fi
